...
import { firstSentences, isoDate } from "../lib/seo.ts";
import { blogImages } from "../data/blog-images.ts";
import { ALT_OVERRIDES } from "./alt-overrides.mjs";
import copyOverrides from "../data/copy-overrides.json" with { type: "json" };
import articleLinks from "../data/article-links.json" with { type: "json" };
...
const pageSchema = { faqs: {}, services: {}, updated: {} };

/* A legal page prints "Last updated <date>"; that date becomes the page's dateModified. */
function statedUpdate(main) {
  const stamp = qa(main, byCls("lg-stamp")).map(text).find((t) => /last updated/i.test(t));
  const m = stamp?.match(/last updated\s+(\d{1,2} [A-Za-z]+ \d{4})/i);
  return m ? m[1] : null;
}
...
async function emitPage(id, { file, replace, attrs, imports = [], noindex = false, faq = false, service = false, component = null } = {}) {
  ...
  const updated = statedUpdate(main);
  if (updated) {
    pageSchema.updated[id] = { name: stripSuffix(TITLES[id]), path: ROUTES[id], dateModified: isoDate(updated) };
    nodes.push(`webPage(updated[${JSON.stringify(id)}])`);
  }
  if (service) {
  ...
  const imp = [
    ...
    nodes.length ? `import { graph${faq ? ", faqPage" : ""}${service ? ", service" : ""}${updated ? ", webPage" : ""} } from "@/lib/jsonld";` : "",
    nodes.length ? `import { ${[faq ? "faqs" : "", service ? "services" : "", updated ? "updated" : ""].filter(Boolean).join(", ")} } from "@/data/page-schema";` : "",
...
/* Internal links from each article (SEO brief, Phase 4.5). Targets are derived, never
   guessed: every category maps to the section of the site that covers it, and case
   studies follow from an industry category or from a named capability. A person can
   add or replace a capability and case studies per article in data/article-links.json,
   which wins over anything derived here. */
const SECTION_BY_TAG = {
  "AI in BFSI": "ind-bfsi",
  "AI in Healthcare": "ind-healthcare",
  "Agentic AI": "svc-agentic",
  "AI Models": "svc-modernisation",
  "AI Tools": "svc-modernisation",
  EsMagico: "who-we-are",
};
const INDUSTRY_KEY_BY_TAG = { "AI in BFSI": "bfsi", "AI in Healthcare": "healthcare" };
...
function relatedFor(post) {
  ...
  const chosen = articleLinks.articles[post.slug] ?? {};

  const sectionId = SECTION_BY_TAG[post.tag];
  if (!sectionId) throw new Error(`no section for category "${post.tag}" (${post.slug})`);

  // a capability the article names in its title, or one a person has chosen
  const named = post.title.match(CAPABILITY)?.[1].toLowerCase();
  const capabilityId = chosen.capability ?? (named ? `pyzo-${named}` : null);

  let studies = [];
  if (chosen.caseStudies) studies = chosen.caseStudies.map((slug) => {
    const c = caseStudiesOut.find((x) => x.slug === slug);
    if (!c) throw new Error(`unknown case study "${slug}" for ${post.slug}`);
    return c;
  });
  else {
    const indKey = INDUSTRY_KEY_BY_TAG[post.tag];
    const cap = capabilityId?.replace("pyzo-", "");
    if (indKey) studies = caseStudiesOut.filter((c) => c.card.industry === indKey);
    else if (cap) studies = caseStudiesOut.filter((c) => c.card.products.some((x) => x.toLowerCase() === cap));
  }
  return {
    section: link(sectionId),
    ...(capabilityId ? { capability: link(capabilityId) } : {}),
...
function addRelated() {
  ...
  for (const post of blogPostsOut) {
    ...
    if (!r.capability || !r.caseStudies.length)
      unmapped.push({ slug: post.slug, tag: post.tag, section: r.section.label, missing: [!r.capability && "capability", !r.caseStudies.length && "case study"].filter(Boolean).join(", ") });
  }
  write("data/blog-posts.ts", tsExport("blogPosts", "BlogPost[]", blogPostsOut));
  const rows = unmapped.map((u) => "| `/blog/" + u.slug + "/` | " + u.tag + " | " + u.section + " | " + u.missing + " |").join("\n");
  write(
    ...
    "# Articles that could carry more internal links\n\n" +
      ...
      `${unmapped.length} of ${blogPostsOut.length} articles.\n\n| Article | Category | Section it links to | Could also take |\n|---|---|---|---|\n${rows}\n`,
  );
  log("related links: section on all", blogPostsOut.length, "| could take more:", unmapped.length, "| chosen by hand:", Object.keys(articleLinks.articles).length);
}
...
// pages a human has since written a description for are no longer needed
const written = needsCopy.filter((n) => copyOverrides.descriptions[n.path]);
const stillNeeded = needsCopy.filter((n) => !copyOverrides.descriptions[n.path]);
const rows = stillNeeded.map((n) => "| `" + n.path + "` | " + n.reason + " |").join("\n");
write(
  ...
  "# Pages that need a meta description written by a human\n\n" +
    ...
    `${stillNeeded.length} pages still need one; ${written.length} have been written in data/copy-overrides.json.\n\n| Path | Why |\n|---|---|\n${rows}\n`,
);
write(
  "data/page-schema.ts",
  `// Generated by tools/convert.mjs from the page copy. Do not edit by hand.\nimport type { Faq } from "@/lib/jsonld";\n\n` +
    ...
    `/** Pages that print their own "Last updated" date */\nexport const updated: Record<string, { name: string; path: string; dateModified: string }> = ${JSON.stringify(pageSchema.updated, null, 2)};\n`,
);
log("needs copy:", stillNeeded.length, "written:", written.length);
log("done");
